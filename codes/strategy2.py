...
class strategy(object):
 ...
 def end_month(self):
  ...
  for i in range(0,end.year-start.year):#simulating dates of the time series
    
    for j in range(1,13):
        
     if i==end.year-start.year-1 and j==12:  # for the last month of the period we don't generate signals since there no data available after this month
        break  
     if j!=12:#excluding December months
       x=df1.iloc[datetime.date(year=start.year+i,month=j,day=28),'Adj Close']# assign price of a stock on the 28th day of current month
       y=df1.iloc[datetime.date(year=start.year+i,month=j+1,day=3),'Adj Close']# assign price of a stock on the third day of following month
       if np.isnan(x)==0 and np.isnan(y)==0:#if these values are not missing
                
          signals.append(1)# buy signal for every month
          prices.append(df1.ix[datetime.date(year=start.year+i,month=j,day=28),'Adj Close']) # buyng the stock 4 days beore the end of the month
          signals.append(-1)# generating sell signal
          prices.append(df1.ix[datetime.date(year=start.year+i,month=j+1,day=3),'Adj Close']) # selling signal on the third day of the next month
       
     else:
       x=df1.iloc[datetime.date(year=start.year+i,month=12,day=28),'Adj Close']
       y=df1.iloc[datetime.date(year=start.year+i+1,month=1,day=3),'Adj Close'] # price of a stock on January 3 of folloiwng month
       if np.isnan(x)==0 and np.isnan(y)==0 and j==12:# In December we buy stock on the 3d day of January
           signals.append(1)# buy signal for every month
           prices.append(df1.ix[datetime.date(year=start.year+i,month=j,day=28),'Adj Close']) # buyng the stock 4 days before the end of the month
           signals.append(-1)# generating sell signal
           prices.append(df1.ix[datetime.date(year=start.year+i+1,month=1,day=3),'Adj Close']) # selling signal on January 3
